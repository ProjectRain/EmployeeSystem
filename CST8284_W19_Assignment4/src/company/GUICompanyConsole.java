...
//		 TODO #1: write getters for the four other text boxes
		...
		// TODO #2: write setters for the four other text boxes
		 public void setNumberToTextArea(String n) {txtNumber.setText(n);}
		 public void setStartDateToTextArea(String date) { txtStartDate.setText(date);}
		 public void setSalaryToTextArea(String d) {txtSalary.setText(d);}
		 public void setExtraInfoToTextArea(String ex) {txtExtraInfo.setText(ex);}
		...
		private void loadButtonHandlers(JFrame frame) {
			...
			addTempEmployeeBtn.addActionListener(event -> {
				try {
			      ...
			      getStartDateFromTextArea(), getSalaryFromTextArea(), date );
					startUp.getEmployees().add(emp);
				} catch (Exception e) {
					JOptionPane.showMessageDialog(frame, e.getMessage());
				}
			...
			loadFromFileBtn.addActionListener(event -> {
				try {
					...
					int currentEmployeeValue = startUp.getCurrentEmployeeValue();
					//System.out.println("In Next -- Current Employee: " + currentEmployeeValue);
					Employee nextEmployee = startUp.getCurrentEmployee();
					txtName.setText(nextEmployee.getName());
					txtNumber.setText(Integer.toString(nextEmployee.getEmployeeNumber()));
					txtStartDate.setText(nextEmployee.getStartDate().toString());
